...
class MLService {
  ...
  List _preProcess(CameraImage image, Face faceDetected) {
    ...
    Float32List imageAsList = img.buffer.asFloat32List();
    // _imageToByteListFloat32(img);
    return imageAsList;
  }
  ...
  //   for (var i = 0; i < 112; i++) {
  //     for (var j = 0; j < 112; j++) {
  //       int pixel = image.getPixel(j, i).current.;
  //       .getPixel(j, i);
  //       buffer[pixelIndex++] =imglib.
  //        ((pixel. & 0xff) - 128) / 128;
  //       buffer[pixelIndex++] = (((pixel >> 8) & 0xff) - 128) / 128;
  //       buffer[pixelIndex++] = (((pixel >> 16) & 0xff) - 128) / 128;
  //     }
  //   }
